...
class WishService
{
    ...
    {
        // Scope theo event_id — tránh token cross-event
        $guest = $token ? Guest::where('token', $token)->where('event_id', $event->id)->first() : null;

        // Anonymous: tạo guest tạm nếu chưa có
        // Fallback name nếu token sai + name null (validation nullable cho token mode)
        if (!$guest) {
